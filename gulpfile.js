...
function scripts() {
  return src('src/js/*.js', { allowEmpty: true }) // Находим все файлы .js в папке src/js
    .pipe(uglify()) // Минифицируем каждый файл
    .pipe(rename({ suffix: '.min' })) // Добавляем суффикс .min к каждому файлу
